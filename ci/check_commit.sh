...
wget https://github.com/${TRAVIS_REPO_SLUG}/releases/download/${RELEASE}/commit-${GIT_BRANCH}-${OS}.hash -O /tmp/commit-${GIT_BRANCH}.hash
...
cp /tmp/commit-${GIT_BRANCH}-new.hash ./commit-${GIT_BRANCH}-${OS}.hash
